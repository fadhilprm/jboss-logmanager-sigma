...
public final class Logger extends java.util.logging.Logger implements Serializable {
	...
	/**
	 * {@inheritDoc} This implementation grabs a lock, so that only one thread
	 * may update the log level of any logger at a time, in order to allow
	 * readers to never block (though there is a window where retrieving the log
	 * level reflects an older effective level than the actual level).
	 */
	...
	/**
	 * Set the log level by name. Uses the parent logging context's name
	 * registry; otherwise behaves identically to {@link #setLevel(Level)}.
	 *
	...
	/**
	 * Get the attachment value for a given key, or {@code null} if there is no
	 * such attachment.
	 *
	 * @param key the key
	...
	/**
	 * Attach an object to this logger under a given key. A strong reference is
	 * maintained to the key and value for as long as this logger exists.
	 *
	...
	/**
	 * Attach an object to this logger under a given key, if such an attachment
	 * does not already exist. A strong reference is maintained to the key and
	 * value for as long as this logger exists.
	 *
	 * @param key the attachment key
	 * @param value the attachment value
	 * @param <V> the attachment value type
	 * @return the current attachment, if there is one, or {@code null} if the
	 * value was successfully attached
	 * @throws SecurityException if a security manager exists and if the caller
	 * does not have {@code LoggingPermission(control)}
	 */
	...
	/**
	 * A convenience method to atomically replace the handler list for this
	 * logger.
	 *
	 ...
	 * @return {@code true} if the handler list was updated or {@code false} if
	 * the current handlers did not match the expected handlers list
	 * @throws SecurityException if a security manager exists and if the caller
	 * does not have {@code LoggingPermission(control)}
	 */
	 ...
	 * <p>
	 * Setting this value to {@code false} has the same behaviour as
	 * {@linkplain java.util.logging.Logger}.
	 * </p>
	 *
	 * @param useParentFilter {@code true} to inherit a parents filter,
	...
	/**
	 * Indicates whether or not this logger inherits filters from it's parent
	 * logger.
	 *
	...
	public void log(LogRecord record) {
		...
		if (!(LogManager.PER_THREAD_LOG_FILTER && (filter = LogManager.getThreadLocalLogFilter()) != null)
				&& (record.getLevel().intValue() < effectiveLevel || effectiveLevel == OFF_INT)) {
			return;
		}
	...
	public void entering(final String sourceClass, final String sourceMethod) {
		Filter filter = null;
		if (!(LogManager.PER_THREAD_LOG_FILTER && (filter = LogManager.getThreadLocalLogFilter()) != null)
				&& FINER_INT < loggerNode.getEffectiveLevel()) {
			return;
		}
	...
	public void severe(final String msg) {
		Filter filter = null;
		if (!(LogManager.PER_THREAD_LOG_FILTER && (filter = LogManager.getThreadLocalLogFilter()) != null)
				&& SEVERE_INT < loggerNode.getEffectiveLevel()) {
			return;
		}
	...
	public void warning(final String msg) {
		Filter filter = null;
		if (!(LogManager.PER_THREAD_LOG_FILTER && (filter = LogManager.getThreadLocalLogFilter()) != null)
				&& WARNING_INT < loggerNode.getEffectiveLevel()) {
			return;
		}
	...
	public void info(final String msg) {
		Filter filter = null;
		if (!(LogManager.PER_THREAD_LOG_FILTER && (filter = LogManager.getThreadLocalLogFilter()) != null)
				&& INFO_INT < loggerNode.getEffectiveLevel()) {
			return;
		}
	...
	public void config(final String msg) {
		Filter filter = null;
		if (!(LogManager.PER_THREAD_LOG_FILTER && (filter = LogManager.getThreadLocalLogFilter()) != null)
				&& CONFIG_INT < loggerNode.getEffectiveLevel()) {
			return;
		}
	...
	public void finest(final String msg) {
		Filter filter = null;
		if (!(LogManager.PER_THREAD_LOG_FILTER && (filter = LogManager.getThreadLocalLogFilter()) != null)
				&& FINEST_INT < loggerNode.getEffectiveLevel()) {
			return;
		}
	...
	public void log(final Level level, final String msg) {
		...
		final int effectiveLevel = loggerNode.getEffectiveLevel();
		if (!(LogManager.PER_THREAD_LOG_FILTER && (filter = LogManager.getThreadLocalLogFilter()) != null)
				&& (level.intValue() < effectiveLevel || effectiveLevel == OFF_INT)) {
			return;
		}
	...
	/**
	 * SPI interface method to log a message at a given level, with a specific
	 * resource bundle.
	 *
	...
	public void log(final String fqcn, final Level level, final String message, final String bundleName,
		...
		if (level == null
				|| fqcn == null
				|| message == null
				|| (!(LogManager.PER_THREAD_LOG_FILTER && (filter = LogManager.getThreadLocalLogFilter()) != null) && (level
						.intValue() < effectiveLevel || effectiveLevel == OFF_INT))) {
			return;
		}
	...
	/**
	 * Do the logging with no level checks (they've already been done). Creates
	 * an extended log record if the provided record is not one.
	 *
