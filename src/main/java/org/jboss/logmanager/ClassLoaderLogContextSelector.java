...
public final class ClassLoaderLogContextSelector implements LogContextSelector {
	...
	private static final Permission UNREGISTER_LOG_CONTEXT_PERMISSION = new RuntimePermission("unregisterLogContext",
			null);

	private static final Permission LOG_API_PERMISSION = new RuntimePermission("logApiPermission", null);

	/**
	 * Construct a new instance. If no matching log context is found, the
	 * provided default selector is consulted.
	 *
	 * @param defaultSelector the selector to consult if no matching log context
	 * is found
	 */
	 ...
	 * provided default selector is consulted.
	 * <p/>
	 * If the {@code checkParentClassLoaders} is set to {@code true} this
	 * selector recursively searches the class loader parents until a match is
	 * found or a {@code null} parent is found.
	 *
	 * @param defaultSelector the selector to consult if no matching log context
	 * is found
	 * @param checkParentClassLoaders {@code true} if the
	 * {@link org.jboss.logmanager.LogContext log context} could not found for
	 * the class loader and the {@link ClassLoader#getParent() parent class
	...
	/**
	 * Construct a new instance. If no matching log context is found, the system
	 * context is used.
	 */
	...
	private final Set<ClassLoader> logApiClassLoaders = Collections
			.newSetFromMap(new CopyOnWriteMap<ClassLoader, Boolean>());
	...
	/**
	 * {@inheritDoc} This instance will consult the call stack to see if any
	 * calling classloader is associated with any log context.
	 */
	...
	/**
	 * Register a class loader which is a known log API, and thus should be
	 * skipped over when searching for the log context to use for the caller
	 * class.
	 *
	 * @param apiClassLoader the API class loader
	 * @return {@code true} if this class loader was previously unknown, or
	 ...
	 *
	 * @param apiClassLoader the API class loader
	 * @return {@code true} if the class loader was removed, or {@code false} if
	 * it was not known to this selector
	 */
	...
	/**
	 * Register a class loader with a log context. This method requires the
	 * {@code registerLogContext} {@link RuntimePermission}.
	 *
	 * @param classLoader the classloader
	 * @param logContext the log context
	 * @throws IllegalArgumentException if the classloader is already associated
	 * with a log context
	 */
	public void registerLogContext(ClassLoader classLoader, LogContext logContext) throws IllegalArgumentException {
		...
		if (contextMap.putIfAbsent(classLoader, logContext) != null) {
			throw new IllegalArgumentException("ClassLoader instance is already registered to a log context ("
					+ classLoader + ")");
		}
	}

	/**
	 * Unregister a class loader/log context association. This method requires
	 * the {@code unregisterLogContext} {@link RuntimePermission}.
	 *
	 * @param classLoader the classloader
	 * @param logContext the log context
	 * @return {@code true} if the association exists and was removed,
