...
/**
 * An extended log record, which includes additional information including
 * MDC/NDC and correct caller location (even in the presence of a logging
 * facade).
 */
public class ExtLogRecord extends LogRecord {
	...
	public enum FormatStyle {
		...
		/**
		 * Format the message using the {@link java.text.MessageFormat}
		 * parameter style.
		 */
		MESSAGE_FORMAT,
		/**
		 * Format the message using the {@link java.util.Formatter} (also known
		 * as {@code printf()}) parameter style.
		 */
		PRINTF,
		/**
	...
	/**
	 * Construct a new instance. Grabs the current NDC immediately. MDC is
	 * deferred.
	 *
	...
	/**
	 * Wrap a JDK log record. If the target record is already an
	 * {@code ExtLogRecord}, it is simply returned. Otherwise a wrapper record
	...
	/**
	 * Disable caller calculation for this record. If the caller has already
	 * been calculated, leave it; otherwise set the caller to {@code "unknown"}.
	 */
	...
	/**
	 * Copy all fields and prepare this object to be passed to another thread or
	 * to be serialized. Calling this method more than once has no additional
	 * effect and will not incur extra copies.
	 */
	public void copyAll() {
		copyMdc();
		calculateCaller();
	...
	/**
	 * Copy the MDC. Call this method before passing this log record to another
	 * thread. Calling this method more than once has no additional effect and
	 * will not incur extra copies.
	 */
	public void copyMdc() {
		if (mdcCopy == null) {
	...
	/**
	 * Get a copy of all the MDC properties for this log record. If the MDC has
	 * not yet been copied, this method will copy it.
	 *
	...
	/**
	 * Change an MDC value on this record. If the MDC has not yet been copied,
	...
	/**
	 * Remove an MDC value on this record. If the MDC has not yet been copied,
	 * this method will copy it.
	 *
	...
	/**
	 * Find the first stack frame below the call to the logger, and populate the
	 * log record with that information.
	 */
	 ...
	 * <p/>
	 * Note that this line number is not verified and may be spoofed. This
	 ...
	 * describe an earlier call on the stack frame. May be -1 if no information
	 * could be obtained.
	 *
	 ...
	 * <p/>
	 * Note that this file name is not verified and may be spoofed. This
	 * information may either have been provided as part of the logging call, or
	 * it may have been inferred automatically by the logging framework. In the
	 * latter case, the information may only be approximate and may in fact
	 * describe an earlier call on the stack frame. May be {@code null} if no
	...
	/**
	 * Get the fully formatted log record, with resources resolved and
	 * parameters applied.
	 *
	...
	/**
	 * Get the resource key, if any. If the log message is not localized, then
	...
	/**
	 * Set the raw message. Any cached formatted message is discarded. The
	 * parameter format is set to be {@link java.text.MessageFormat}-style.
	 *
	...
	/**
	 * Set the raw message. Any cached formatted message is discarded. The
	 * parameter format is set according to the given argument.
	 *
	 * @param message the new raw message
	...
	/**
	 * Set the parameters to the log message. Any cached formatted message is
	 * discarded.
	 *
	...
	/**
	 * Set the localization resource bundle. Any cached formatted message is
	 * discarded.
	 *
	...
	/**
	 * Set the localization resource bundle name. Any cached formatted message
	 * is discarded.
	 *
