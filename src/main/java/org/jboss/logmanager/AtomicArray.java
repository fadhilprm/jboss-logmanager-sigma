 ...
 * of {@link java.util.concurrent.atomic.AtomicReferenceFieldUpdater
 * AtomicReferenceFieldUpdater} which corresponds to that field. Some of these
 * methods perform multi-step operations; if the array field value is changed in
 * the middle of such an operation, the operation is retried. To avoid spinning,
 * in some situations it may be advisable to hold a write lock to prevent
...
final class AtomicArray<T, V> {
	 ...
	 * @return {@code true} if the value was updated or {@code false} if the
