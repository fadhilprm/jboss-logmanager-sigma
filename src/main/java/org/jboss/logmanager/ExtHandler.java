...
/**
 * An extended logger handler. Use this class as a base class for log handlers
 * which require {@code ExtLogRecord} instances.
 */
public abstract class ExtHandler extends Handler implements FlushableCloseable, Protectable {
	...
	private static final AtomicReferenceFieldUpdater<ExtHandler, Object> protectKeyUpdater = AtomicReferenceFieldUpdater
			.newUpdater(ExtHandler.class, Object.class, "protectKey");

	/**
	 * The sub-handlers for this handler. May only be updated using the
	 * {@link #handlersUpdater} atomic updater. The array instance should not be
	 * modified (treat as immutable).
	 */
	@SuppressWarnings({})
	 ...
	 */
	protected static final AtomicArray<ExtHandler, Handler> handlersUpdater = AtomicArray.create(
			AtomicReferenceFieldUpdater.newUpdater(ExtHandler.class, Handler[].class, "handlers"), Handler.class);
	 ...
	 * <p/>
	 * The {@code ExtHandler} is responsible for formatting the message, when
	...
	/**
	 * Do the actual work of publication; the record will have been filtered
	 * already. The default implementation does nothing except to flush if the
	 * {@code autoFlush} property is set to {@code true}; if this behavior is to
	 * be preserved in a subclass then this method should be called after the
	 * record is physically written.
	 *
	 * @param record the log record to publish
	 */
	...
	/**
	 * Add a sub-handler to this handler. Some handler types do not utilize
	 * sub-handlers.
	 *
	 * @param handler the handler to add
	 *
	 * @throws SecurityException if a security manager exists and if the caller
	 * does not have {@code LoggingPermission(control)} or the handler is
	 * {@link #protect(Object) protected}.
	 */
	...
	/**
	 * Remove a sub-handler from this handler. Some handler types do not utilize
	 * sub-handlers.
	 *
	...
	/**
	 * Get a copy of the sub-handlers array. Since the returned value is a copy,
	 * it may be freely modified.
	 *
	 ...
	 *
	 * @param autoFlush {@code true} to automatically flush after each write;
	 * false otherwise
	 *
	 * @throws SecurityException if a security manager exists and if the caller
	 * does not have {@code LoggingPermission(control)} or the handler is
	 ...
	 *
	 * @param enabled {@code true} to enable the handler or {@code false} to
	 * disable the handler.
	 *
	 * @throws SecurityException if a security manager exists and if the caller
	 * does not have {@code LoggingPermission(control)} or the handler is
	 ...
	 * caller does not have the {@code "control" LoggingPermission}
	 */
	@Deprecated
