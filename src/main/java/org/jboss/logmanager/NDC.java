...
/**
 * Nested diagnostic context. This is basically a thread-local stack that holds
 * a string which can be included in a log message.
 */
public final class NDC {
	...
	/**
	 * Push a value on to the NDC stack, returning the new stack depth which
	 * should later be used to restore the stack.
	 *
	...
	/**
	 * Trim the thread NDC stack down to no larger than the given size. Used to
	 * restore the stack to the depth returned by a {@code push()}.
	 *
	...
	/**
	 * Provided for compatibility with log4j. Get the NDC value that is
	 * {@code n} entries from the bottom.
	 *
	 * @param n the index
